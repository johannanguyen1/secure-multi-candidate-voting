...
/**
 * serialize Vote_Ciphertext_Wrapper
 */
void Vote_Ciphertext_Wrapper::serialize(std::vector<unsigned char> &data) {
  // Add message type.
  data.push_back((char)MessageType::Vote_Ciphertext_Wrapper);

  size_t vote_vector_size = this->vote_vector.size();
  put_integer(CryptoPP::Integer(vote_vector_size), data);

  //Add vote ciphertext
  for (size_t i = 0; i < vote_vector_size; i++) {
    std::vector<unsigned char> vote_data;
    this->vote_vector[i].serialize(vote_data);
    data.insert(data.end(), vote_data.begin() + 1, vote_data.end());
  }
}

/**
 * deserialize Vote_Ciphertext_Wrapper
 */
int Vote_Ciphertext_Wrapper::deserialize(std::vector<unsigned char> &data) {
  assert(data[0] == MessageType::Vote_Ciphertext_Wrapper);

  int n = 1;
  CryptoPP::Integer vote_vector_size;
  n += get_integer(&vote_vector_size, data, n);

  for (int i = 0; i < vote_vector_size; i++){
    std::vector<unsigned char> vote_slice(data.begin() + n, data.end());
    vote_slice.insert(vote_slice.begin(), (char)MessageType::Vote_Ciphertext);
    
    Vote_Ciphertext vote;
    int bytes_read = vote.deserialize(vote_slice);
    n += (bytes_read - 1); 
    
    vote_vector.push_back(vote);
  }
  return n;
}
...
/**
 * serialize a VoteZKP_Wrapper
 */
void VoteZKP_Wrapper::serialize(std::vector<unsigned char> &data){
  data.push_back((char)MessageType::VoteZKP_Wrapper);
  put_integer(CryptoPP::Integer(CANDIDATE_NUMBER), data);
  for (VoteZKP_Struct &zkp : zkp_vector) {
    zkp.serialize(data);
  }
}
/**
 * deserialize a VoteZKP_Wrapper
 */
int VoteZKP_Wrapper::deserialize(std::vector<unsigned char> &data){
  assert(data[0] == MessageType::VoteZKP_Wrapper);

  CryptoPP::Integer num_candidates;
  int n = 1;
  n += get_integer(&num_candidates, data, n);

  for (int i = 0; i < CANDIDATE_NUMBER; ++i) {
    std::vector<unsigned char> zkp_slice(data.begin() + n, data.end());
    VoteZKP_Struct zkp;
    int bytes_read = zkp.deserialize(zkp_slice);
    n += bytes_read; 
    zkp_vector.push_back(zkp);
  }
  return n;
}
...
void VoterToTallyer_Vote_Message::serialize(std::vector<unsigned char> &data) {
  ...
  std::vector<unsigned char> vote_data;
  this->vote_wrapper.serialize(vote_data);
  ...
  this->zkp_wrapper.serialize(zkp_data);
...
int VoterToTallyer_Vote_Message::deserialize(std::vector<unsigned char> &data) {
  ...
  n += this->vote_wrapper.deserialize(vote_slice);

  n += get_integer(&this->unblinded_signature, data, n);

  std::vector<unsigned char> zkp_slice =
      std::vector<unsigned char>(data.begin() + n, data.end());
  n += this->zkp_wrapper.deserialize(zkp_slice);
...
void PartialDecryption_Struct::serialize(std::vector<unsigned char> &data) {
  ...
  //Get d vector size
  put_integer(CryptoPP::Integer(d_vector.size()), data);

  // Add d vector.
  for (CryptoPP::Integer& d : d_vector) {
    put_integer(d, data);
  }

  //add aggregate ciphertext
...
int PartialDecryption_Struct::deserialize(std::vector<unsigned char> &data) {
  ...
  // Get fields.
  int n = 1;
  CryptoPP::Integer d_vector_size;
  n += get_integer(&d_vector_size, data, n);

  for (int i = 0; i < d_vector_size; ++i) {
    CryptoPP::Integer d;
    n += get_integer(&d, data, n);
...
concat_vote_zkp_and_signature(Vote_Ciphertext_Wrapper &vote, VoteZKP_Wrapper &zkp,
