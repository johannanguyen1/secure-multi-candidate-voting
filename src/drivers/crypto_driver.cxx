...
std::pair<CryptoPP::Integer, CryptoPP::Integer> CryptoDriver::EG_generate() {
  ...
  Integer a(rng, 2, DL_Q-1);
  // exponentiate base DL_G to get public value
  Integer public_value = CryptoPP::ModularExponentiation(DL_G, a, DL_Q);
  return std::pair(a, public_value);
}
...
std::pair<RSA::PrivateKey, RSA::PublicKey> CryptoDriver::RSA_generate_keys() {
  ...
  return std::pair<RSA::PrivateKey, RSA::PublicKey>(private_key, public_key);
}
...
std::string CryptoDriver::RSA_sign(const RSA::PrivateKey &signing_key,
  ...
  StringSource ss1(string_message, true, new SignerFilter(rng, signer, new StringSink(signature)));
...
CryptoDriver::RSA_BLIND_sign(const RSA::PrivateKey &private_key,
  ...
  // TODO: implement me!
  AutoSeededRandomPool rng;
  Integer signature = private_key.CalculateInverse(rng, blinded_msg); //TODO: find out what's wrong with this
...
CryptoDriver::RSA_BLIND_unblind(const RSA::PublicKey &public_key,
  ...
  // TODO: implement me!
  Integer n = public_key.GetModulus();
  Integer mm = a_times_b_mod_c(signed_blind_msg, blind.InverseMod(n), n);
...
bool CryptoDriver::RSA_BLIND_verify(const RSA::PublicKey &public_key,
  ...
  Integer check = ModularExponentiation(signature, e, n);
