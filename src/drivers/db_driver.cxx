  ...
  while (sqlite3_step(stmt) == SQLITE_ROW) {
    ...
    for (int colIndex = 0; colIndex < sqlite3_column_count(stmt); colIndex++) {
      ...
      case 0:
        ...
        vote.vote_wrapper.deserialize(data);
        break;
      case 1:
        data = str2chvec(std::string((const char *)raw_result, num_bytes));
        vote.zkp_wrapper.deserialize(data);
        break;
      case 2:
 ...
 */
VoteRow DBDriver::find_vote(Vote_Ciphertext_Wrapper vote_s) {
  // Lock db driver.
  std::unique_lock<std::mutex> lck(this->mtx);
...
VoteRow DBDriver::insert_vote(VoteRow vote) {
  ...
  std::vector<unsigned char> vote_data;
  vote.vote_wrapper.serialize(vote_data);
  std::string vote_str = chvec2str(vote_data);

  std::vector<unsigned char> zkp_data;
  vote.zkp_wrapper.serialize(zkp_data);
 ...
 */
bool DBDriver::vote_exists(Vote_Ciphertext_Wrapper vote) {
  // Lock db driver.
  std::unique_lock<std::mutex> lck(this->mtx);
