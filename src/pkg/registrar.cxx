...
void RegistrarClient::HandleRegister(
  ...
  // gets user info
  std::vector<unsigned char> raw_user_data;
  raw_user_data = network_driver->read();
  auto user_data = crypto_driver->decrypt_and_verify(AES_key, HMAC_key, raw_user_data).first;
  VoterToRegistrar_Register_Message user_info;
  user_info.deserialize(user_data);

  RegistrarToVoter_Blind_Signature_Message blind_signature_message;  
  blind_signature_message.id = user_info.id;
  // verifies that the user hasn't already registered
  if (this->db_driver->find_voter(user_info.id).id == user_info.id){
    this->cli_driver->print_warning("User has already registered.");
    blind_signature_message.registrar_signature = this->db_driver->find_voter(user_info.id).registrar_signature;
  } else {
    this->cli_driver->print_info("New voter has been determined eligable to vote.");
    // blindly signs message 
    blind_signature_message.registrar_signature = crypto_driver->RSA_BLIND_sign(this->RSA_registrar_signing_key, user_info.vote);
    // adds user to database
    VoterRow voter;
    voter.id = user_info.id;
    voter.registrar_signature = blind_signature_message.registrar_signature;
    this->db_driver->insert_voter(voter);
  }
  // sends to user
  std::vector<unsigned char> blind_signature_data = crypto_driver->encrypt_and_tag(AES_key, HMAC_key, &blind_signature_message);
  network_driver->send(blind_signature_data); 
  // --------------------------------
  // Exit cleanly
