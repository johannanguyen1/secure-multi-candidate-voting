...
void TallyerClient::HandleTally(std::shared_ptr<NetworkDriver> network_driver,
  ...
  // recieve a vote from the user
  std::vector<unsigned char> raw_user_vote_data = network_driver->read();
  auto user_vote_data = crypto_driver->decrypt_and_verify(AES_key, HMAC_key, raw_user_vote_data).first;
  VoterToTallyer_Vote_Message user_vote;
  user_vote.deserialize(user_vote_data);

  // make sure the user hasn't voted yet

  if (this->db_driver->vote_exists(user_vote.vote_wrapper)){
    throw std::runtime_error("Voter has already voted.");
    network_driver->disconnect();
    return;
  }

  Vote_Ciphertext_Wrapper multiple_votes = user_vote.vote_wrapper;
  VoteZKP_Wrapper multiple_zkps = user_vote.zkp_wrapper; 

  Integer unblind_sig = user_vote.unblinded_signature;

  if (multiple_votes.vote_vector.size() != CANDIDATE_NUMBER) {
        throw std::runtime_error("Number of candidates voted for is invalid.");
  }                           

  bool zkps_valid = true;
  if (!ElectionClient::VerifyVoteZKPs(
    std::make_pair(multiple_votes, multiple_zkps), this->EG_arbiter_public_key)) {
      zkps_valid = false;
      this->cli_driver->print_warning("Failed zkp verification in tallyer"); 
  }

  // TODO: verify sum proof 

  // verify the server's signature
  if (!crypto_driver->RSA_BLIND_verify(
    this->RSA_registrar_verification_key, multiple_votes, unblind_sig)) {
      throw std::runtime_error("Invalid unblinded signature.");
      network_driver->disconnect();
      return;
  }
  ...
  multiple_votes.serialize(serialized_vote);
  std::string tallyer_signature = crypto_driver->RSA_sign(
    this->RSA_tallyer_signing_key, serialized_vote);

  // publishes to the database and marks the user as having already voted
  VoteRow vote_row;
  vote_row.tallyer_signature = tallyer_signature;
  vote_row.unblinded_signature = unblind_sig;
  vote_row.vote_wrapper = multiple_votes;
  vote_row.zkp_wrapper = multiple_zkps;
